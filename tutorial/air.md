TODO: Say something about inference not work quite as expected. (We've
followed the paper as best we can, but we might be missing some
details.)
...
steps, each of which generates a small part of the whole. We'll call
these parts 'objects' for simplicity. In the case of AIR applied to
...
The model also includes uncertainty about the position and scale of
each part, and the number of steps used for each data point.
...
The core of the model is the generative process for a single step.
Recall that:
...
additively with "image so far" generated by previous steps.
...
that avoid this redundant computation.)

Putting this all together, the generative process looks something like
this:
...
TODO: Show samples form the prior.
...
a whole as a recurrent network. At each step the network will generate
...
The description above is one perspective on the model, that comes from
thinking about its implementation in Pyro. Another interesting
perspective is to zoom out and look at the model and guide computation
as a whole. When we do so we see that at each time step AIR includes a
sub-computation that has the same structure as a Variational
Auto-encoder. From this perspective, the act of cropping the input
image serves to restrict the attention of model to a small region of
...
discrete choices in the AIR model makes inference trickier than in a
model in which all choices are continuous.

(TODO: Link to tutorial discussion of combined estimator, trickiness
of reinforce.)

The underlying problem we face is that the gradients estimates we use
in the optimization performed by variational inference have much
higher variance in the presence of discrete choices.

The paper uses two approaches to bring the variance under control and
obtain good results from inference.

## Baselines

Baselines are one approach to variance reduction. To get a feel for
how they work, we need to look a little more closely at how gradient
estimates are made for discrete choices.
...
Of course, the raise the question of what value we ought to choose for
`b_i` in order to reduce the variance of our estimates?
...
as the baselines network and the guide network are two separate
