...
class NormalNormalTests(TestCase):
    ...
    def _test_nested_map_data_in_elbo(self, n_steps=10000):
    ...
    def test_vectorized_map_data_in_elbo(self, n_steps=1):
        pyro.get_param_store().clear()
        self.data = Variable(torch.randn(5,2))
        ...
        def model2():
            ...
            def obs_outer(i, x):
                pyro.map_data("map_obs_inner_%d" % i, x, lambda _i, _x:
                              obs_inner(_i, _x), batch_size=3)

            def obs_inner(_i, _x):
                pyro.observe("obs", dist.diagnormal, _x, mu_latent,
                             torch.pow(self.lam, -0.5))

            pyro.map_data("map_obs_outer", [self.data], lambda i, x:
                          obs_outer(i, x), batch_size=1)
        ...
        def model():
            ...
            def obs_outer(i, x):
                pyro.observe("obs", dist.diagnormal, x, mu_latent,
                             torch.pow(self.lam, -0.5))
