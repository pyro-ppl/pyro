...
from pyro.infer import SVI, TraceGraph_ELBO, JitTraceGraph_ELBO
...
class GaussianPyramidTests(TestCase):
    ...
    def model(self, loc0, lambdas, target_lambdas, data, reparameterized, model_permutation, difficulty=0.0):
        top_latent_dist = dist.Normal(loc0, torch.pow(lambdas[0], -0.5))
        ...
        for n in range(2, self.N + 1):
            ...
            for prev_latent, prev_name in permute(previous_latents_and_names, n - 1):
                latent_dist = dist.Normal(prev_latent, torch.pow(lambdas[n - 1], -0.5))
                couple = []
                for LR in ['L', 'R']:
                    new_name = prev_name + LR
        ...
        for i, data_i in enumerate(data):
            for k, x in enumerate(data_i):
                pyro.sample("obs_%s_%d" % (previous_latents_and_names[i][1], k),
                            dist.Normal(previous_latents_and_names[i][0], torch.pow(lambdas[-1], -0.5)),
                            obs=x)
        return top_latent

    def guide(self, loc0, lambdas, target_lambdas, data, reparameterized, model_permutation, difficulty=0.0):
        latents_dict = {}
        ...
        for i, node in enumerate(self.q_topo_sort):
            ...
            log_sig_node = pyro.param("log_sig_" + node_suffix,
                                      torch.tensor(-0.5 * torch.log(target_lambdas[node_suffix]).clone().detach() +
                                                   difficulty * (torch.Tensor([-0.3]) -
                                                                 0.3 * (torch.randn(1) ** 2)),
                                                   requires_grad=True))
            mean_function_node = pyro.param("constant_term_" + node,
                                            torch.tensor(loc0.clone().detach() +
    ...
    def do_elbo_test(self, reparameterized, n_steps, lr, prec, beta1,
        ...
        target_lambda_keys = list(self.target_lambdas.keys())
        target_lambda_values = [self.target_lambdas[k] for k in target_lambda_keys]

        def model_closure(loc0, lambdas, target_lambda_values, data):
            target_lambdas = dict(zip(target_lambda_keys, target_lambda_values))
            return self.model(loc0, lambdas, target_lambdas, data,
                              reparameterized=reparameterized,
                              model_permutation=model_permutation,
                              difficulty=difficulty)

        def guide_closure(loc0, lambdas, target_lambda_values, data):
            target_lambdas = dict(zip(target_lambda_keys, target_lambda_values))
            return self.guide(loc0, lambdas, target_lambdas, data,
                              reparameterized=reparameterized,
                              model_permutation=model_permutation,
                              difficulty=difficulty)
        ...
        if self.N == 2:
            guide_trace = pyro.poutine.trace(self.guide,
                                             graph_type="dense").get_trace(self.loc0, self.lambdas, self.target_lambdas, self.data,
                                                                           reparameterized=reparameterized,
                                                                           model_permutation=model_permutation,
                                                                           difficulty=difficulty)
        ...
        svi = SVI(model_closure, guide_closure, adam, loss=JitTraceGraph_ELBO())

        for step in range(n_steps):
            t0 = time.time()
            svi.step(self.loc0, self.lambdas, target_lambda_values, self.data)
                # reparameterized=reparameterized, model_permutation=model_permutation, difficulty=difficulty)
