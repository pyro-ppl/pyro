...
def test_negative_binomial_vs_poisson(R0, I):
    ...
    d1 = infection_dist(individual_rate=R0, num_infectious=I)
    d2 = infection_dist(individual_rate=R0, num_infectious=I, concentration=200.0)
...
def test_overdispersed_asymptote(probs, overdispersion):
    ...
    k = torch.arange(0.0, total_count + 1.0)
...
@pytest.mark.parametrize("concentration1", [0.2, 1.0, 5.0])
@pytest.mark.parametrize("concentration0", [0.2, 1.0, 5.0])
...
def test_overdispersed_beta_binomial(probs, total_count, overdispersion):
    ...
    concentration = 100.0  # very little uncertainty
