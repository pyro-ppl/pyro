...
def TraceEnum_ELBO(*args, **kwargs):
    global _ENUM_BACKEND_VERSION
    return (FunsorEnum_ELBO if _ENUM_BACKEND_VERSION == "funsor" else PyroEnum_ELBO)(*args, **kwargs)


def TraceTMC_ELBO(*args, **kwargs):
    global _ENUM_BACKEND_VERSION
    return (FunsorTMC_ELBO if _ENUM_BACKEND_VERSION == "funsor" else PyroTMC_ELBO)(*args, **kwargs)
...
def test_tmc_categoricals(depth, max_plate_nesting, num_samples, tmc_strategy):
    ...
    def model():
        ...
        with pyro_plate("local", 3):
            for i in range(1, depth):
                x = pyro.sample("x{}".format(i),
                                dist.Categorical(pyro.param("q{}".format(i))[..., x, :]))
            with pyro_plate("data", 4):
                pyro.sample("y", dist.Bernoulli(pyro.param("qy")[..., x]),
                            obs=data)

    with toggle_backend("pyro"):
        elbo = TraceTMC_ELBO(max_plate_nesting=max_plate_nesting)
