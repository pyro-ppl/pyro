...
def test_sample(dist):
    for idx in range(len(dist.dist_params)):
        ...
        try:
            cpu_value = dist.pyro_dist_class(**params).sample()
        ...
        with tensors_default_to("cuda"):
            params = dist.get_dist_params(idx)
        cuda_value = dist.pyro_dist_class(**params).sample()
...
def test_log_pdf(dist):
    for idx in range(len(dist.dist_params)):
        ...
        with xfail_if_not_implemented():
            cpu_value = dist.pyro_dist_class(**params).log_pdf(data)
        assert not cpu_value.is_cuda
        ...
        with tensors_default_to("cuda"):
            data = dist.get_test_data(idx)
            params = dist.get_dist_params(idx)
        cuda_value = dist.pyro_dist_class(**params).log_pdf(data)
        assert cuda_value.is_cuda
...
def test_batch_log_pdf(dist):
    for idx in range(len(dist.dist_params)):
        ...
        with tensors_default_to("cpu"):
            ...
            params = dist.get_dist_params(idx)
        with xfail_if_not_implemented():
            cpu_value = dist.pyro_dist_class(**params).log_prob(data)
        assert not cpu_value.is_cuda
        ...
        with tensors_default_to("cuda"):
            data = dist.get_test_data(idx)
            params = dist.get_dist_params(idx)
        cuda_value = dist.pyro_dist_class(**params).log_prob(data)
        assert cuda_value.is_cuda
