...
def test_rejector(rate, factor):
    num_samples = 100000
    rates = torch.tensor(rate).expand(num_samples, 1).requires_grad_(True)
    factors = torch.tensor(factor).expand(num_samples, 1).requires_grad_(True)
...
def test_standard_gamma_elbo(alpha):
    num_samples = 100000
    alphas = torch.tensor(alpha).expand(num_samples, 1).requires_grad_(True)
...
def test_gamma_elbo(alpha, beta):
    num_samples = 100000
    alphas = torch.tensor(alpha).expand(num_samples, 1).requires_grad_(True)
    betas = torch.tensor(beta).expand(num_samples, 1).requires_grad_(True)
