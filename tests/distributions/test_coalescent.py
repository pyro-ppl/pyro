...
@pytest.mark.parametrize("batch_shape", [(), (6,), (2, 3)])
@pytest.mark.parametrize("sample_shape", [(), (7,), (4, 5)])
@pytest.mark.parametrize("num_leaves", [2, 3, 5, 10, 100])
def test_shape(num_leaves, num_steps, batch_shape, sample_shape):
    ...
    shape = sample_shape + (1,) * len(batch_shape) + (num_steps,)
