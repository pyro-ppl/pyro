...
def test_elbo_enumerate_plate_5():
    ...
    def model_plate():
        ...
        with pyro.plate("b_axis", 2):
            b = pyro.sample("b", dist.Categorical(probs_b))
            pyro.sample("c", dist.Categorical(broadcasted(probs_c)[a, b]),
                        obs=data)
    ...
    def model_iplate():
        ...
        for i in pyro.plate("b_axis", 2):
            ...
            pyro.sample("c_{}".format(i),
                        dist.Categorical(broadcasted(probs_c)[a, b]),
                        obs=data[i])
...
def test_elbo_enumerate_plate_7(scale):
    ...
    def auto_model(data):
        ...
        with pyro.plate("data", 2):
            ...
            d = pyro.sample("d", dist.Categorical(broadcasted(probs_d)[b, c]),
                            infer={"enumerate": "parallel"})
    ...
    def hand_model(data):
        ...
        for i in pyro.plate("data", 2):
            ...
            d = pyro.sample("d_{}".format(i),
                            dist.Categorical(broadcasted(probs_d)[b, c]),
                            infer={"enumerate": "parallel"})
        ...
        for i in b_axis:
            for j in c_axis:
                pyro.sample("d_{}_{}".format(i, j),
                            dist.Categorical(broadcasted(probs_d)[b[i], c[j]]),
                            obs=data[i, j])
        ...
        for i in b_axis:
            with c_axis:
                pyro.sample("d_{}".format(i),
                            dist.Categorical(broadcasted(probs_d)[b[i], c]),
                            obs=data[i])
        ...
        with b_axis:
            for j in c_axis:
                pyro.sample("d_{}".format(j),
                            dist.Categorical(broadcasted(probs_d)[b, c[j]]),
                            obs=data[:, j])
