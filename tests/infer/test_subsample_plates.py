...
def test_iarange_ok(subsample_size, Elbo):
    ...
    def model():
        p = torch.tensor(0.5)
        with SubsampleMessenger("iarange", 10, subsample_size) as ind:
...
def test_iarange_no_size_ok(Elbo):
    ...
    def model():
        p = torch.tensor(0.5)
        with SubsampleMessenger("iarange"):
...
def test_iarange_broadcast_error(Elbo, is_validate):
    ...
    def model():
        ...
        with SubsampleMessenger("iarange", 10, 5):
...
def test_nested_iarange_iarange_ok(Elbo):
    ...
    def model():
        p = torch.tensor(0.5, requires_grad=True)
        with SubsampleMessenger("iarange_outer", 10, 5) as ind_outer:
            pyro.sample("x", dist.Bernoulli(p).expand_by([len(ind_outer)]))
            with SubsampleMessenger("iarange_inner", 11, 6) as ind_inner:
...
def test_iarange_reuse_ok(Elbo):
    ...
    def model():
        p = torch.tensor(0.5, requires_grad=True)
        iarange_outer = SubsampleMessenger("iarange_outer", 10, 5, dim=-1)
        iarange_inner = SubsampleMessenger("iarange_inner", 11, 6, dim=-2)
...
def test_nonnested_iarange_iarange_ok(Elbo):
    ...
    def model():
        p = torch.tensor(0.5, requires_grad=True)
        with SubsampleMessenger("iarange_0", 10, 5) as ind1:
            pyro.sample("x0", dist.Bernoulli(p).expand_by([len(ind1)]))
        with SubsampleMessenger("iarange_1", 11, 6) as ind2:
...
def test_iarange_wrong_size_error():
    ...
    def model():
        p = torch.tensor(0.5)
        with SubsampleMessenger("iarange", 10, 5) as ind:
...
def test_enum_discrete_parallel_iarange_ok():
    ...
    def model():
        ...
        with SubsampleMessenger("outer", 3):
            x34 = pyro.sample("x34", dist.Categorical(p34))
            with SubsampleMessenger("inner", 5):
...
def test_enum_discrete_iaranges_dependency_ok(enumerate_):
    ...
    def model():
        pyro.sample("w", dist.Bernoulli(0.5), infer={'enumerate': 'parallel'})
        x_iarange = SubsampleMessenger("x_iarange", 10, 5, dim=-1)
        y_iarange = SubsampleMessenger("y_iarange", 11, 6, dim=-2)
...
def test_enum_discrete_non_enumerated_iarange_ok(enumerate_):
    ...
    def model():
        ...
        with SubsampleMessenger("non_enum", 2):
        ...
        with SubsampleMessenger("enum_1", 3):
...
def test_iarange_shape_broadcasting(times):
    ...
    def model():
        with SubsampleMessenger("num_particles", 10, dim=-3):
            with SubsampleMessenger("components", 2, dim=-1):
                p = pyro.sample("p", dist.Beta(torch.tensor(1.1), torch.tensor(1.1)))
                assert p.shape == torch.Size((10, 1, 2))
            with SubsampleMessenger("data", data.shape[0], dim=-2):
...
def test_enum_discrete_iarange_shape_broadcasting_ok(enumerate_):
    ...
    def model():
        x_iarange = SubsampleMessenger("x_iarange", 10, 5, dim=-1)
        y_iarange = SubsampleMessenger("y_iarange", 11, 6, dim=-2)
        with SubsampleMessenger("num_particles", 50, dim=-3):
            with x_iarange:
...
def test_dim_allocation_ok(Elbo):
    ...
    def model():
        p = torch.tensor(0.5, requires_grad=True)
        with SubsampleMessenger("iarange_outer", 10, 5, dim=-3):
            x = pyro.sample("x", dist.Bernoulli(p))
            assert x.shape == torch.Size((5, 1, 1))
            with SubsampleMessenger("iarange_inner_1", 11, 6):
                y = pyro.sample("y", dist.Bernoulli(p))
                # allocated dim is rightmost available, i.e. -1
                assert y.shape == torch.Size((5, 1, 6))
                with SubsampleMessenger("iarange_inner_2", 12, 7):
                    ...
                    with SubsampleMessenger("iarange_inner_3", 13, 8):
...
def test_dim_allocation_error(Elbo):
    ...
    def model():
        p = torch.tensor(0.5, requires_grad=True)
        with SubsampleMessenger("iarange_outer", 10, 5, dim=-2):
            x = pyro.sample("x", dist.Bernoulli(p))
            ...
            with SubsampleMessenger("iarange_inner_1", 11, 6):
                ...
                with SubsampleMessenger("iarange_inner_2", 12, 7, dim=-1):
...
def test_vectorized_num_particles(Elbo):
    ...
    @poutine.broadcast
    def model():
        with SubsampleMessenger("components", 2):
            p = pyro.sample("p", dist.Beta(torch.tensor(1.1), torch.tensor(1.1)))
            assert p.shape == torch.Size((10, 1, 2))
            with SubsampleMessenger("data", data.shape[0]):
                pyro.sample("obs", dist.Bernoulli(p), obs=data)
