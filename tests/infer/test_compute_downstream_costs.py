    ...
    for node in guide_nodes:
        ...
        for desc in descendants:
            desc_mft = MultiFrameTensor(
                (
                    stacks[desc],
                    model_trace.nodes[desc]["log_prob"] - guide_trace.nodes[desc]["log_prob"],
                )
            )
        ...
        with pyro.plate("plate_triple1", 6) as ind_triple1:
            with pyro.plate("plate_triple2", 7) as ind_triple2:
                if include_z1:
                    pyro.sample(
                        "z1",
                        dist.Bernoulli(p2).expand_by([len(ind_triple2), len(ind_triple1)]),
                    )
                with pyro.plate("plate_triple3", 9) as ind_triple3:
                    pyro.sample(
                        "z0",
                        dist.Bernoulli(p2).expand_by([len(ind_triple3), len(ind_triple2), len(ind_triple1)]),
                    )
...
def test_compute_downstream_costs_big_model_guide_pair(
    ...
    with TrackNonReparam():
        ...
        model_trace = poutine.trace(poutine.replay(big_model_guide, trace=guide_trace), graph_type="dense").get_trace(
            include_obs=True,
            include_inner_1=include_inner_1,
            include_single=include_single,
            flip_c23=flip_c23,
...
def test_compute_downstream_costs_duplicates(dim):
    with TrackNonReparam():
        ...
        model_trace = poutine.trace(poutine.replay(diamond_model, trace=guide_trace), graph_type="dense").get_trace(
            dim=dim
        )
