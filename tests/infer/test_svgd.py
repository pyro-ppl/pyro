...
@pytest.mark.parametrize("latent_dist", [#dist.TransformedDistribution(dist.Normal(torch.zeros(1), torch.ones(1)),
    ...
    #dist.Normal(torch.zeros(1) + 0.5, torch.ones(1)).to_event(1)])#,
                                         dist.LogNormal(torch.tensor([0.0]), torch.tensor([1.0])).to_event(1)])
...
def test_shapes(shape):
    ...
    def model():
        pyro.sample("z", dist.Normal(torch.zeros(shape), 1.0).to_event(len(shape)))

    svgd = SVGD(model, SVGDRBFKernel(), Adam({"lr": 0.001}), 7, 0)
