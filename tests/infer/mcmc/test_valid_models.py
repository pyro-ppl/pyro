...
def test_log_prob_eval_iterates_in_correct_order():
    ...
    def model():
        ...
        with outer, inner2:
            ...
            pyro.sample("obs0", dist.Normal(2 * z0 - 1, 1.0), obs=torch.ones(5, 3))
        with outer:
...
def test_all_discrete_sites_log_prob(Eval):
    ...
    assert_equal(trace_prob_evaluator.log_prob(model_trace), torch.tensor(0.0))
...
def test_enumeration_in_tree(Eval):
    ...
    @poutine.condition(
        data={
            "sample1": torch.tensor(0.0),
            "sample2": torch.tensor(1.0),
            "sample3": torch.tensor(2.0),
        }
    )
    def model():
...
@pytest.mark.parametrize(
    "data, expected_log_prob",
    [
        (torch.tensor([1.0]), torch.tensor(-1.3434)),
        (torch.tensor([0.0]), torch.tensor(-1.4189)),
        (torch.tensor([1.0, 0.0, 0.0]), torch.tensor(-4.1813)),
    ],
)
@pytest.mark.parametrize("Eval", [TraceTreeEvaluator, TraceEinsumEvaluator])
def test_enum_log_prob_continuous_observed(data, expected_log_prob, Eval):
    ...
    def model(data):
        ...
        with pyro.plate("data", len(data)):
            ...
            mean = 2 * z - 1
            pyro.sample("obs", dist.Normal(mean, 1.0), obs=data)
...
@pytest.mark.parametrize(
    "data, expected_log_prob",
    [
        (torch.tensor([1.0]), torch.tensor(-3.5237)),
        (torch.tensor([0.0]), torch.tensor(-3.7091)),
        (torch.tensor([1.0, 1.0]), torch.tensor(-3.9699)),
        (torch.tensor([1.0, 0.0, 0.0]), torch.tensor(-5.3357)),
    ],
)
@pytest.mark.parametrize("Eval", [TraceTreeEvaluator, TraceEinsumEvaluator])
def test_enum_log_prob_continuous_sampled(data, expected_log_prob, Eval):
    ...
    def model(data):
        ...
        y = pyro.sample("y", dist.Bernoulli(p))
        mean = 2 * y - 1
        n = pyro.sample("n", dist.Normal(mean, 1.0))
        with pyro.plate("data", len(data)):
...
@pytest.mark.parametrize(
    "data, expected_log_prob",
    [
        (torch.tensor([1.0]), torch.tensor(-0.5108)),
        (torch.tensor([1.0, 1.0]), torch.tensor(-0.9808)),
        (torch.tensor([1.0, 0.0, 0.0]), torch.tensor(-2.3671)),
    ],
)
@pytest.mark.parametrize("Eval", [TraceTreeEvaluator, TraceEinsumEvaluator])
...
@pytest.mark.parametrize(
    "data, expected_log_prob",
    [
        (torch.tensor([1.0]), torch.tensor(-1.15)),
        (torch.tensor([0.0]), torch.tensor(-1.46)),
        (torch.tensor([1.0, 1.0]), torch.tensor(-2.1998)),
    ],
)
@pytest.mark.parametrize("Eval", [TraceTreeEvaluator, TraceEinsumEvaluator])
...
@pytest.mark.parametrize(
    "data, expected_log_prob",
    [
        (torch.tensor([1.0]), torch.tensor(-1.5478)),
        (torch.tensor([0.0]), torch.tensor(-1.4189)),
        (torch.tensor([1.0, 0.0, 0.0]), torch.tensor(-4.3857)),
    ],
)
@pytest.mark.parametrize("Eval", [TraceTreeEvaluator, TraceEinsumEvaluator])
    ...
    def model(data):
        ...
        with pyro.plate("intermediate", 1, dim=-2):
            ...
            with pyro.plate("data", len(data), dim=-1):
                ...
                pyro.sample("obs", dist.Normal(2 * z - 1, 1.0), obs=data)
