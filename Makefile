...
test-cuda: lint FORCE
	CUDA_TEST=1 PYRO_DTYPE=torch.float64 PYRO_DEVICE=cuda pytest -vx --stage unit
	CUDA_TEST=1 pytest -vx tests/test_examples.py::test_cuda

test-cuda-lax: lint FORCE
	CUDA_TEST=1 PYRO_DTYPE=torch.float64 PYRO_DEVICE=cuda pytest -vx --stage unit --lax
	CUDA_TEST=1 pytest -vx tests/test_examples.py::test_cuda
