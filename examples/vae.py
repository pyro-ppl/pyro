...
class VAE(nn.Module):
    ...
    def model(self, x):
        ...
        z = pyro.sample("latent", dist.Normal(z_mu, z_sigma).reshape(extra_event_dims=2))
        # decode the latent code z
        mu_img = self.decoder.forward(z)
        # score against actual images
        pyro.sample("obs", dist.Bernoulli(mu_img).reshape(extra_event_dims=2), obs=x.view(-1, 784))
    ...
    def guide(self, x):
        ...
        pyro.sample("latent", dist.Normal(z_mu, z_sigma).reshape(extra_event_dims=2))
