...
def model(observed_data):
    ...
    pyro.map_data("map", observed_data, lambda i, x: observe_T(x, "obs_%d" % i), batch_size=10)
    return mu
...
def guide(observed_data):
    ...
    pyro.map_data("map", observed_data, lambda i, x: None, batch_size=10)
    return mu


# do stochastic variational inference using the elbo objective
...
def main():
    ...
    for step in range(args.num_epochs):
        ...
        if step % 5 == 0:
            print("[epoch %03d] mean_mu:  %.3f" % (step, pyro.param("mean_mu").data[0]))
    ...
    print(("disregarding measurement noise, elementary calulus gives the descent time\n" +
