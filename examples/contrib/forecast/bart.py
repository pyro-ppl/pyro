...
class Model(ForecastingModel):
    ...
    def model(self, zero_data, covariates):
        ...
        noise_scale = pyro.sample("noise_scale", dist.LogNormal(torch.full((dim,), -3.0), 1.0).to_event(1))
        assert noise_scale.shape[-1:] == (dim,)
        trans_timescale = pyro.sample("trans_timescale", dist.LogNormal(torch.zeros(dim), 1).to_event(1))
        ...
        prediction = periodic_repeat(season_init, duration, dim=-2) + periodic_cumsum(season_noise, period, dim=-2)
