...
class Z2LEncoder(nn.Module):
    ...
    def forward(self, x, s):
        ...
        z2_loc, z2_scale = h1[..., :-1], softplus(h2[..., :-1] - 2.0)
        l_loc, l_scale = h1[..., -1:], softplus(h2[..., -1:] - 2.0)
...
class Spatial(nn.Module):
    def __init__(self, num_genes, num_labels, latent_dim=5, alpha=0.01, scale_factor=1.0):
    ...
    def model(self, l_mean, l_scale, x, s, y=None):
        ...
        with pyro.plate("batch", len(x)), poutine.scale(scale=self.scale_factor):
            ...
            xx = x_dist.sample()
            print("xx", xx.sum(-1).mean().item(), "x", x.sum(-1).mean().item())
...
def main(args):
    ...
    optim = ClippedAdam({"lr": args.learning_rate, "clip_norm": 0.02})
