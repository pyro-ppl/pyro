...
class VAE(nn.Module):
    ...
    def model(self, observations=None):
        ...
        for x_id in pyro.plate("data", x.shape[0]):
            # setup hyperparameters for prior p(z)
            z_loc = torch.zeros(1, self.z_dim, dtype=x.dtype, device=x.device)
            z_scale = torch.ones(1, self.z_dim, dtype=x.dtype, device=x.device)
            # sample from prior (value will be sampled by guide when computing the ELBO)
            z = pyro.sample("latent_{}".format(x_id), dist.Normal(z_loc, z_scale).to_event(1))
            # decode the latent code z
            loc_img = self.decoder.forward(z)
            # score against actual images
            # pyro.sample("obs", dist.Bernoulli(loc_img).to_event(1), obs=x.reshape(-1, 784))
            pyro.sample("obs_{}".format(x_id), dist.Bernoulli(loc_img).to_event(1), obs=x[x_id].reshape(-1, 784))
    ...
    def guide(self, observations=None):
        ...
        for x_id in pyro.plate("data", x.shape[0]):
            ...
            z_loc, z_scale = self.encoder.forward(x[x_id].reshape(-1, 784))
            # sample the latent code z
            pyro.sample("latent_{}".format(x_id), dist.Normal(z_loc, z_scale).to_event(1))
...
def main(args):
    ...
    rws = pyro.infer.ReweightedWakeSleep(num_particles=10, vectorize_particles=vectorize_particles, insomnia=1.)
    iwae = pyro.infer.RenyiELBO(alpha=0, num_particles=10, vectorize_particles=vectorize_particles)
    elbo = rws if use_rws else iwae
    print(elbo)
    # elbo = JitTrace_ELBO() if args.jit else Trace_ELBO()
    get_loss = lambda loss: loss[0] if use_rws else loss
