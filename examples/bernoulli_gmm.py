...
def local_model(i, datum):
    ...
    cll = pyro.sample("class_of_datum_" + str(i), Bernoulli(beta))
    mean_param = Variable(torch.zeros(1,784), requires_grad=True)
    # do MLE for class means
    m = pyro.param("mean_of_class_" + str(cll[0]), mean_param)
...
for epoch in range(nr_epochs):
    ...
    for i in range(nr_samples):
        ...
        loss_sample = grad_step(i, data[i].view(1,-1))
