...
class SSVAE(nn.Module):
    ...
    def model(self, xs, ys=None):
        ...
        with pyro.iarange("independent"):
            ...
            zs = pyro.sample("z", dist.Reshape(dist.Normal(prior_mu, prior_sigma), extra_event_dims=1))
            ...
            pyro.sample("x", dist.Reshape(dist.Bernoulli(mu), extra_event_dims=1), obs=xs)
    ...
    def guide(self, xs, ys=None):
        ...
        with pyro.iarange("independent"):
            ...
            pyro.sample("z", dist.Reshape(dist.Normal(mu, sigma), extra_event_dims=1))
    ...
    def model_sample(self, ys, batch_size=1):
        ...
        mu = self.decoder.forward([zs, ys])
        xs = pyro.sample("sample", dist.Reshape(dist.Bernoulli(mu), extra_event_dims=1))
        return xs, mu
