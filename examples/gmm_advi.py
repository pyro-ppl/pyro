...
def model(K, N, D, y, alpha0, alpha0_vec):
    ...
    sigma = pyro.sample("sigma", dist.LogNormal(torch.ones(K,D), torch.ones(K,D))) #.abs()
...
def run_stan(data):
    with open("gmm_diag.stan","r") as f:
        code=f.read()
...
if __name__== "__main__":
    ...
    (K, N, D, y, alpha0), data = get_data("training.data.json", varnames)
