...
from torch.distributions.transforms import Transform as TorchTransform
from torch.distributions.utils import _sum_rightmost
from pyro.distributions import Transform
...
class TransformedDistribution(Distribution):
    ...
    def log_prob(self, value, **kwargs):
        ...
        for transform in reversed(self.transforms):
            if isinstance(transform, Transform):
                x = transform.inv(y, **kwargs)
                log_prob = log_prob - _sum_rightmost(transform.log_abs_det_jacobian(x, y, **kwargs),
    ...
    def cdf(self, value, **kwargs):
        ...
        for transform in self.transforms[::-1]:
            if isinstance(transform, Transform):
                value = transform.inv(value, **kwargs)
            else:
