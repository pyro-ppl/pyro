...
class HeterogeneousMixture(Distribution):
    ...
    def sample(self):
        ...
        return parts[ind]  # FIXME this does not reparameterize wrt categorical components.

    def batch_log_pdf(self, x):
        parts = torch.stack([c.batch_log_pdf(x) for c in self._components], -1)
        log_probs = self._categorical.batch_log_pdf(self._categorical.enumerate_support())
