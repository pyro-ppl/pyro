...
class ProgressBar(object):
    ...
    def num_lines(self):
        ...
        return int(math.ceil(len(str(self)) / screen_width)) if screen_width not in (None, 0) else 1
    ...
    def __str__(self):
        ...
        return self.desc + " [" + self.progbar.ljust(self._width) + "] " + "({})".format(postfix)
...
class MultiProgressBars(object):
    ...
    @property
    def num_lines(self):
        return sum(p.num_lines for p in self.progbars)
...
p = MultiProgressBars()
p.add(total=100, desc="hello", postfix="bye")
p.add(total=100, desc="second", postfix="bye")
for i in range(100):
    sleep(0.2)
    p.increment(pos=0)
    p.increment(pos=1)
