...
# size is the number of valid points in case we use slice sampling
...
class NUTS(HMC):
    ...
    **References**
    ...
    :param bool use_multinomial_sampling: A flag to decide if we want to sample
        ...
        while multinomial sampling is suggested in [2].
    ...
    def _build_basetree(self, z, r, z_grads, log_slice, direction, energy_current):
        ...
        # This is obtained by setting energy_new to +inf when it is nan.
        ...
        if self.use_multinomial_sampling:
            ...
            tree_size = (sliced_energy.new_ones(()) if sliced_energy <= 0
    ...
    def _build_tree(self, z, r, z_grads, log_slice, direction, tree_depth, energy_current):
        ...
        if self.use_multinomial_sampling:
            # TODO: for pytorch 1.0, we don't need to check these conditions
            if half_tree.size != float("-inf") or other_half_tree != float("-inf"):
                tree_size = logsumexp(torch.stack([half_tree.size, other_half_tree.size]), dim=0)
            else:
                tree_size = half_tree.size  # -inf
        else:
        ...
        if self.use_multinomial_sampling:
            other_half_tree_prob = ((other_half_tree.size - tree_size).exp()
                                    if tree_size != float("-inf") else tree_size.new_zeros(()))
        else:
            other_half_tree_prob = (other_half_tree.size / tree_size if tree_size != 0
                                    else tree_size.new_zeros(()))
        is_other_half_tree = pyro.sample(
            "is_other_halftree", dist.Bernoulli(probs=other_half_tree_prob))
    ...
    def sample(self, trace):
        ...
        r_sum = r_flat
        if self.use_multinomial_sampling:
            tree_size = energy_current.new_zeros(())
        else:
            tree_size = energy_current.new_ones(())
        ...
        with optional(pyro.validation_enabled(False), self._adapt_phase):
            ...
            for tree_depth in range(self._max_tree_depth + 1):
                ...
                if self.use_multinomial_sampling:
                    new_tree_prob = (new_tree.size - tree_size).exp()
                else:
                    new_tree_prob = new_tree.size / tree_size
                ...
                if self._is_turning(r_left, r_right, r_sum):  # stop doubling
                    ...
                    if self.use_multinomial_sampling:
                        tree_size = logsumexp(torch.stack([tree_size, new_tree.size]), dim=0)
                    else:
                        tree_size = tree_size + new_tree.size
