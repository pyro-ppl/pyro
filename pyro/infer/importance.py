...
class LogWeightsMixin:
    ...
    def get_log_normalizer(self):
        ...
        if len(self.log_weights) > 0:
            log_w = self.log_weights if isinstance(self.log_weights, torch.Tensor) else torch.tensor(self.log_weights)
...
def vectorized_importance_weights(model, guide, *args, **kwargs):
    ...
    if num_samples == 1:
        ...
        log_weights = plate_log_prob_sum(model_trace, wd) - plate_log_prob_sum(guide_trace, wd)
