...
def psis_diagnostic(model, guide, *args, **kwargs):
    ...
    Finally note that this implementation is adapted from the implementation
    in reference [2].
    ...
    :param max_simultaneous_particles: the maximum number of simulteanous samples drawn from the
    ...
    log_weights = [vectorized_importance_weights(model, guide, num_samples=num_particles,
                                                 ...
                                                 *args, **kwargs)[0].data.cpu().numpy() for _ in range(N)]
