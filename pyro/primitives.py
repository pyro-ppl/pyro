...
def factor(name, log_factor, *, has_rsample=False):
    """
    ...
    .. warning:: Beware using factor statements in guides. Factor statements
        assume by default that ``log_factor`` is computed from
        non-reparametrized statements such as observation statements
        ``pyro.sample(..., obs=...)``. If instead ``log_factor`` is computed
        from e.g. the Jacobian determinant of a transformation of a
        reparametrized variable, you'll need to set ``has_rsample=True``.
    ...
    :param bool has_rsample: Whether the ``log_factor`` arose from a fully
        reparametrized distribution. Defaults to False, which is safe for use
        in models (but may not be safe for use in guides).
    """
