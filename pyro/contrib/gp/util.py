    ...
    def _register_param(self, param):
        ...
        if param in self._priors:
            if self.mode == "model":
                ...
                # NOTE: here we define Delta guide (initiated randomly from prior) but
                # the code can be modified to support guides as in `contrib.autoguide` module.
                p_MAP = pyro.param(param_name + "_MAP", self._priors[param])
                p = pyro.sample(param_name, dist.Delta(p_MAP))
        else:
            p = pyro.param(param_name, default_value, constraint=self._constraints[param])
