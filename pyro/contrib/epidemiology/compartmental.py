...
class CompartmentalModel(ABC):
    ...
    def fit_svi(
        ...
        **options,
        ...
        heuristic_options = {
            k.replace("heuristic_", ""): options.pop(k) for k in list(options) if k.startswith("heuristic_")
        }
    ...
    def _heuristic(self, haar, **options):
        ...
        logger.info(
            "Heuristic init: {}".format(
                ", ".join("{}={:0.3g}".format(k, v.item()) for k, v in sorted(init_values.items()) if v.numel() == 1)
            )
        )
