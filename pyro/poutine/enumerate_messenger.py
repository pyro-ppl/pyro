...
class EnumerateMessenger(Messenger):
    ...
    def _pyro_sample(self, msg):
        ...
        generalized_slice = getattr(value, '_pyro_generalized_slice', None)
        if generalized_slice is not None:
            # Preserve generalized_slices to speed up Categorical.log_prob().
            # See pyro/ops/indexing.py and pyro/distributions/torch.py for details.
            assert target_dim < 0
            value = value.reshape(value.shape[:1] + (1,) * (-1 - target_dim))
            value._pyro_generalized_slice = generalized_slice
