...
def log_likelihood(
    ...
    :type pointwise: bool, optional
    :return: Returns a function which to which you can subsequently pass model
        input data, that returns either a 1D or 2D tensor of log likelihood
        values depending on the value of the `pointwise` argument.
    :rtype: Callable
    """
    ...
    def log_like_helper(*args, **kwargs) -> torch.Tensor:
        ...
        if parallel:
            ...
            for obs_name, obs_val in observed.items():
                ...
                log_like.append(obs_site["fn"].log_prob(obs_val).detach().cpu())
            log_like = torch.stack(log_like, dim=0).sum(dim=0)
        # iterate over samples from posterior if model can't be vectorized
            ...
            for i in range(num_samples):
                ...
                for obs_name, obs_val in observed.items():
                    obs_site = trace.nodes[obs_name]
                    sample_ll.append(obs_site["fn"].log_prob(obs_val).detach().cpu())
                log_like.append(torch.stack(sample_ll, dim=0).sum(dim=0))
            log_like = torch.stack(log_like, dim=0)
        if pointwise:
            return log_like
        return log_like.sum(dim=1)
